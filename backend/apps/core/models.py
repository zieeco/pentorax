...
class UserProfile(TimeStampedModel):
    ...
    role: models.CharField = models.CharField(
        max_length=50, choices=ROLE_CHOICES, default="customer"
    )
    avatar_url: models.URLField = models.URLField(
        blank=True, null=True, default="", help_text="Profile Image URL"
    )
    bio: models.TextField = models.TextField(blank=True, null=True, default="")
    ...
    def __str__(self):
        return (
            f"{self.name} ({self.user.email if self.user else 'No User'}) - {self.role}"
        )
...
class CaseStudy(TimeStampedModel):
    ...
    capacity: models.CharField = models.CharField(
        max_length=100, help_text="e.g., 250kW"
    )
    savings: models.CharField = models.CharField(
        max_length=200, help_text="e.g., 65% reduction in energy costs"
    )
    project_date: models.DateField = models.DateField(null=True, blank=True)
...
class FAQ(TimeStampedModel):
    ...
    question: models.CharField = models.CharField(max_length=500)
...
class TeamMember(TimeStampedModel):
    ...
    name: models.CharField = models.CharField(max_length=255)
    role: models.CharField = models.CharField(max_length=255)
    department: models.CharField = models.CharField(
        max_length=50, choices=DEPARTMENT_CHOICES
    )
    bio: models.TextField = models.TextField(blank=True, null=True, default="")
    image: models.URLField = models.URLField(max_length=500, help_text="Storage URL")
    email: models.EmailField = models.EmailField(
        max_length=255, blank=True, null=True, default=""
    )
    linkedin: models.URLField = models.URLField(blank=True, null=True, default="")
