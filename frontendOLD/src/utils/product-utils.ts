...
export function calculateDiscount(price: string | number, comparePrice: string | number): number {
  ...
  const comparePriceNum = typeof comparePrice === 'string' ? parseFloat(comparePrice) : comparePrice;
 ...
 */
export function getStockStatusVariant(stockQuantity: number, isLowStock: boolean): 'destructive' | 'outline' | 'default' {
 ...
 */
export function validateProductForm(data: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
