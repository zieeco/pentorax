...
const BatteriesPage: React.FC = () => {
  // Fetch products from batteries category
      ...
      <div className="bg-gray-50 py-24 min-h-screen">
        <div className="container mx-auto px-4">
          ...
          {!isLoading && !isError && products.length === 0 && (
            <div className="text-center py-20">
              <p className="text-gray-500 mb-4">No batteries available at the moment.</p>
              <Link to="/shop" className="text-primary hover:underline">
                Browse all products
              </Link>
            </div>
                ...
                <div key={product.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all border border-gray-100 group">
                  <Link to={`/shop/${product.slug}`} className="block">
                    <div className="h-64 overflow-hidden relative">
                      <img 
                        src={product.featured_image || 'https://via.placeholder.com/400x300?text=Battery'} 
                        alt={product.name} 
